...
//	<!--Get username, firstname, lastname, email, password, password2-->
//Get username
...
$sql = "INSERT INTO users (`first_name`, `last_name`, `username`, `email`, `password`, `activation`, `gender`, `phonenumber`, `moreinformation`) VALUES ('$firstname', '$lastname', '$username', '$email', '$password', '$activationKey', '$gender', '$phonenumber', '$moreinformation')";
$result = mysqli_query($link, $sql);
if(!$result){
	echo "<div class='alert alert-danger'>There was an error inserting the  user's details in the database!</div>";
...
$message .= "http://benswenson.host20.uk/CarShare/activate.php?email=" . urlencode($email) . "&key=$activationKey";
if(mail($email, 'Confirm your Registration', $message, 'From:' . 'info@benswenson.net')){
	echo "<div class='alert alert-success'>Thank you for registering! A confirmatioin email has been sent to $email. Please click on the activation link to activate your account.</div>";
}
